...
public class BloggerSecPage {
	 static WebDriver driver;
	    ...
	    @Test
	    void fullflow() throws InterruptedException {
	    	login();
	    	openApp();
	    	newPage();
	         ...
	         setXYPosition(driver,"10", "70");
              ...
              dragInputBTNToContainer(driver);
              placeAndStyleContainer("#FFFFFF", "100", "%", "40", "px");
              setXYPosition(driver,"0", "13");
              updateInputText("Issue Title");
              
              
              dragInputBTNToContainer(driver);
              setXYPosition(driver,"0", "13");
              placeAndStyleContainer("#FFFFFF", "100", "%", "40", "px");
              setXYPosition(driver,"0", "70");
              updateInputText("Author");
	    ...
	    void newPage() {
	    	 WebElement PageIcon = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[@id='pages']")));
	         PageIcon.click();
	         WebElement createIcon = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//p[contains(text(),'create')]")));
	         createIcon.click();
	    }
	    ...
	    public void dragInputBTNToContainer(WebDriver driver) {
	        WebDriverWait wait = new WebDriverWait(driver, Duration.ofMillis(1000));
	        
	        Actions actions = new Actions(driver);

	        // Wait and locate the draggable tool button from the visual element list
	        WebElement toolInput = wait.until(ExpectedConditions.visibilityOfElementLocated(
	            By.xpath("//div[@class='tool-input']")
	        ));
	        ...
	        actions.moveToElement(toolInput)
	               .clickAndHold()
	 ...
	 public void dragImageBoxToContainer(WebDriver driver) {
	  ...
	  //value of btn box :   
	    static void updateInputText(String newText) {
	        // Step 1: Locate the ao-config element with explicit wait
	        WebElement aoConfig = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("ao-config")));

	        // Step 2: Access the shadow root using JavascriptExecutor (alternative to getShadowRoot)
	        SearchContext shadowRoot = (SearchContext) ((JavascriptExecutor) driver).executeScript("return arguments[0].shadowRoot", aoConfig);

	        // Step 3: Update the config value directly in the textarea
	        WebElement jsonTextarea = shadowRoot.findElement(By.cssSelector("textarea.config-textarea"));
	        String json = "{\n  \"placeholder\": \"" + newText + "\"\n}";
	        jsonTextarea.clear();
	        jsonTextarea.sendKeys(json);

	        // Step 4: Click the summary to apply the change
